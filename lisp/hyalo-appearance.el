;;; hyalo-appearance.el --- Appearance management for hyalo -*- lexical-binding: t -*-
...
;; Appearance management for hyalo.
;; Handles:
...
(defcustom hyalo-appearance-mode-setting 'auto
...
(defcustom hyalo-appearance-opacity 0.5
...
(defun hyalo-appearance-current ()
  "Return the effective appearance as `light' or `dark'.
Based on `hyalo-appearance-mode-setting'.
If mode is `auto', queries the system appearance."
  (pcase hyalo-appearance-mode-setting
    ('light 'light)
    ('dark 'dark)
    ('auto (hyalo-appearance--get-system-appearance))
    (_ 'light)))

(defun hyalo-appearance-dark-p ()
  "Return non-nil if current appearance is dark."
  (eq (hyalo-appearance-current) 'dark))

;;; Core Application Functions

(defun hyalo-appearance--apply-vibrancy ()
...
This is the core function for applying visual settings."
  (when (hyalo-available-p)
    ...
    (when (fboundp 'hyalo-sidebar-set-vibrancy-material)
      (hyalo-sidebar-set-vibrancy-material hyalo-appearance-vibrancy-material))
    ;; Apply opacity (tint layer) with current background color
    (let* ((is-dark (hyalo-appearance-dark-p))
...
(defun hyalo-appearance--on-system-change (appearance)
...
Only reacts if appearance mode is `auto'."
  ...
  (when (eq hyalo-appearance-mode-setting 'auto)
    ;; Load appropriate theme (only in auto mode)
    (let ((theme (if (eq appearance 'dark)
                     hyalo-appearance-theme-dark
                   hyalo-appearance-theme-light)))
      (when theme
...
Does NOT alter vibrancy settings."
  ...
  (when (hyalo-available-p)
    ...
    (when (and hyalo-appearance--initialized
               theme (symbolp theme))
      (setq hyalo-appearance-current-theme theme)
...
(defun hyalo-appearance-set (appearance &optional no-save)
...
Always saves to custom.el unless NO-SAVE is non-nil."
  ...
  (setq hyalo-appearance-mode-setting appearance)
  ;; Persist to custom.el by default
  (unless no-save
    (customize-save-variable 'hyalo-appearance-mode-setting appearance))
  ;; Apply frame settings
  (hyalo-appearance--apply-frame-settings)
  ;; Get effective appearance
  (let ((effective (hyalo-appearance-current)))
    ...
    (hyalo-appearance--apply-window-appearance effective)
  ...
  (when (hyalo-available-p)
    ...
    (when (fboundp 'hyalo-get-panel-appearance-mode)
      (let ((mode (intern (hyalo-get-panel-appearance-mode))))
        (setq hyalo-appearance-mode-setting mode)
        (when save
          (customize-save-variable 'hyalo-appearance-mode-setting mode))))
    ;; Sync vibrancy material
    (when (fboundp 'hyalo-get-panel-vibrancy-material)
      (let ((material (hyalo-get-panel-vibrancy-material)))
        (setq hyalo-appearance-vibrancy-material material)
        (when save
    ...
    (when (fboundp 'hyalo-get-panel-opacity)
      (let ((opacity (hyalo-get-panel-opacity)))
        (setq hyalo-appearance-opacity opacity)
        (when save
          (customize-save-variable 'hyalo-appearance-opacity opacity))))))

(defun hyalo-appearance-sync-to-panel ()
...
Pushes vibrancy, opacity, and appearance mode to Swift, then refreshes panel."
  ...
  (when (hyalo-available-p)
    ...
    (let* ((bg (or (face-background 'default nil 'default)
                   (if (hyalo-appearance-dark-p) "#282c34" "#fafafa")))
           (hex-color (if (string-prefix-p "#" bg)
                          bg
                        (apply #'format "#%02x%02x%02x"
                               (mapcar (lambda (c) (/ c 256))
    ...
    (when (fboundp 'hyalo-set-panel-appearance-mode)
      (hyalo-set-panel-appearance-mode (symbol-name hyalo-appearance-mode-setting)))
    ;; Refresh panel to display updated values
    (when (fboundp 'hyalo-refresh-appearance-panel)
      (hyalo-refresh-appearance-panel))))

(defun hyalo-appearance-set-opacity (opacity)
...
Applies to all frames without changing the theme."
  ...
  (let ((clamped (max 0.0 (min 1.0 opacity))))
    (setq hyalo-appearance-opacity clamped)
    (hyalo-appearance--apply-vibrancy)
...
Use ultraThin for maximum see-through effect."
  (interactive
   (list (completing-read "Vibrancy material: "
                          ...
                          nil t nil nil hyalo-appearance-vibrancy-material)))
  (setq hyalo-appearance-vibrancy-material material)
  (hyalo-appearance--apply-vibrancy)
  (when (and (hyalo-available-p) (fboundp 'hyalo-refresh-appearance-panel))
    (hyalo-refresh-appearance-panel))
...
(defun hyalo-appearance--start-panel-timer ()
  "Start timer to sync from panel while it's open."
  (hyalo-appearance--stop-panel-timer)
  (setq hyalo-appearance--panel-timer
        (run-with-timer 0.3 0.3 #'hyalo-appearance--panel-tick)))

(defun hyalo-appearance--stop-panel-timer ()
  "Stop the panel sync timer."
  (when (timerp hyalo-appearance--panel-timer)
    (cancel-timer hyalo-appearance--panel-timer)
    (setq hyalo-appearance--panel-timer nil)))

(defun hyalo-appearance--panel-tick ()
...
Syncs to Emacs variables and saves if values differ."
  (if (and (hyalo-available-p)
      ...
      (hyalo-appearance--sync-and-save-if-changed)
    ;; Panel was closed externally - stop timer
    (hyalo-appearance--stop-panel-timer)))

(defun hyalo-appearance--sync-and-save-if-changed ()
  "Sync from panel and save only values that have changed."
  (when (hyalo-available-p)
    ;; Sync and save appearance mode if changed
    (when (fboundp 'hyalo-get-panel-appearance-mode)
      (let ((mode (intern (hyalo-get-panel-appearance-mode))))
        (unless (eq mode hyalo-appearance-mode-setting)
          (setq hyalo-appearance-mode-setting mode)
          (customize-save-variable 'hyalo-appearance-mode-setting mode))))
    ;; Sync and save vibrancy material if changed
    (when (fboundp 'hyalo-get-panel-vibrancy-material)
      (let ((material (hyalo-get-panel-vibrancy-material)))
        (unless (equal material hyalo-appearance-vibrancy-material)
          (setq hyalo-appearance-vibrancy-material material)
          (customize-save-variable 'hyalo-appearance-vibrancy-material material))))
    ;; Sync and save opacity if changed
    (when (fboundp 'hyalo-get-panel-opacity)
      (let ((opacity (hyalo-get-panel-opacity)))
        (unless (equal opacity hyalo-appearance-opacity)
          (setq hyalo-appearance-opacity opacity)
          (customize-save-variable 'hyalo-appearance-opacity opacity))))))

(defun hyalo-appearance-show-panel ()
  "Show the Hyalo appearance panel and start sync timer."
  (interactive)
  (when (hyalo-available-p)
    (hyalo-appearance-sync-to-panel)
    (hyalo-show-appearance-panel)
    (hyalo-appearance--start-panel-timer)))

(defun hyalo-appearance-toggle-panel ()
...
(defun hyalo-appearance-hide-panel ()
...
(defun hyalo-appearance--setup-frame (&optional frame)
  ...
  (let ((f (or frame (selected-frame))))
    (when (and (display-graphic-p f) (eq (framep f) 'ns))
      ...
      (set-frame-parameter f 'right-fringe 4)
      (when (hyalo-available-p)
        (hyalo-appearance--apply-window-appearance
         (hyalo-appearance-current))))))

(defun hyalo-appearance--enable ()
  ...
  (condition-case err
      (progn
        ...
        (let ((appearance (hyalo-appearance-current)))
          ...
          (if (eq hyalo-appearance-mode-setting 'auto)
            ...
            (when hyalo-appearance-current-theme
              (mapc #'disable-theme custom-enabled-themes)
              (load-theme hyalo-appearance-current-theme t)))
        ...
        (when (boundp 'ns-system-appearance-change-functions)
          (add-hook 'ns-system-appearance-change-functions
                    #'hyalo-appearance--on-system-change))
...
(defun hyalo-appearance--disable ()
  ...
  (setq hyalo-appearance--initialized nil)
  ;; Remove hooks
  (when (boundp 'ns-system-appearance-change-functions)
    (remove-hook 'ns-system-appearance-change-functions
                 #'hyalo-appearance--on-system-change))
  (when (boundp 'enable-theme-functions)
    (remove-hook 'enable-theme-functions #'hyalo-appearance--on-theme-load))
...
(define-minor-mode hyalo-appearance-mode
  "Minor mode for hyalo appearance management.
When enabled, manages theme and transparency based on system appearance."
  :global t
  :lighter " ηAppearance"
  :group 'hyalo-appearance
  (if hyalo-appearance-mode
      (hyalo-appearance--enable)
    (hyalo-appearance--disable)))
