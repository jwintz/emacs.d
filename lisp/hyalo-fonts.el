...
(when (fboundp 'fontaine-mode)
  ...
  (defun hyalo|apply-comment-font ()
    ...
    (when (display-graphic-p)
      ...
      (set-face-attribute 'font-lock-comment-delimiter-face nil
                          :family "Monaspace Radon Var"
                          :slant 'normal)))
  (add-hook 'fontaine-set-preset-hook #'hyalo|apply-comment-font)
  (add-hook 'after-init-hook #'hyalo|apply-comment-font))

;; -----------------------------------------------------------------------------
;; Mode-Specific Font Hooks
;; -----------------------------------------------------------------------------

(defun hyalo|agent-shell-fonts ()
...
(defun hyalo|terminal-fonts ()
  "Set terminal buffers to use Monaspace Argon."
  (face-remap-add-relative 'default :family "Monaspace Argon Var"))

;; Install hooks
(with-eval-after-load 'agent-shell
  (add-hook 'agent-shell-mode-hook #'hyalo|agent-shell-fonts))

(add-hook 'eshell-mode-hook #'hyalo|terminal-fonts)
(add-hook 'eat-mode-hook #'hyalo|terminal-fonts)
(add-hook 'term-mode-hook #'hyalo|terminal-fonts)
(add-hook 'vterm-mode-hook #'hyalo|terminal-fonts)
